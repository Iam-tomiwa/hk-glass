...
export function useListFactoryQueue(params?: { order_status?: OrderStatus | any | null }) {
...
export function useUpdateFactoryOrderStatus() {
  ...
  return useMutation({
    mutationFn: ({ order_id, data }: { order_id: string; data: OrderStatusUpdate }) => updateFactoryOrderStatus(order_id, data),
    onSuccess: () => {
